...
describe("handleFocusStart", () => {
  ...
  it("creates session for valid input", async () => {
    vi.mocked(readSession).mockResolvedValueOnce(null);
    ...
    expect(result.content[0].text).toContain(validArgs.task);
    expect(writeSession).toHaveBeenCalled();

    const written = vi.mocked(writeSession).mock.calls[0][1];
    expect(written.task).toBe(validArgs.task);
    expect(written.done_criteria).toEqual(validArgs.done_criteria);
